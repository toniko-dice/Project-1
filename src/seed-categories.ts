...
/** Серия (второ ниво) заедно с подсериите си (трето ниво). */
type Серия = Възел & { subs: Възел[] }
type Главна = Възел & { subs: Серия[] }
...
const прочети = async (): Promise<Главна[]> => {
  ...
  let текущаСерия: Серия | null = null
  ...
  for (const ред of текст.split(/\r?\n/)) {
    ...
    */
    ...
    /*
      Нивото се чете от отстъпа: „- " е серия, „  - " е подсерия под
      последната серия. Подсерия без серия над себе си се пропуска — по-добре
      липсваща категория, отколкото закачена не там, където трябва.
    */
    const точка = ред.match(/^(\s*)-\s+(.+)$/)
    if (!точка || !текуща) continue

    const възел = разбий(точка[2])
    if (!възел) continue

    if (точка[1].length === 0) {
      текущаСерия = { ...възел, subs: [] }
      текуща.subs.push(текущаСерия)
    } else if (текущаСерия) {
      текущаСерия.subs.push(възел)
    }
  }
...
for (const главна of дърво) {
  ...
  for (const серия of главна.subs) {
    плоско.push({ ...серия, parentSlug: главна.slug })
    // Подсериите вървят веднага след серията си — това е и редът в админа.
    for (const под of серия.subs) плоско.push({ ...под, parentSlug: серия.slug })
  }
}

const вДървото = new Set(плоско.map((x) => x.slug))

const броиСерии = дърво.reduce((n, г) => n + г.subs.length, 0)
console.log(
  `Дърво: ${дърво.length} главни, ${броиСерии} серии, ${плоско.length - дърво.length - броиСерии} подсерии.`,
)
  ...
  if (!съществуваща) {
    const doc = await payload.create({
      ...
      data: {
        ...
        /*
          Бележката от `kategorii.md` е за хора, които четат файла („само
          ако dice.bg го продава", „твърди/монтажни") — НЕ е текст за
          посетители. По-рано влизаше в подзаглавието на банера и излизаше
          под заглавието на категорията. Подзаглавието и описанието ги
          пише собственикът от админа.
        */
        _order: ключ,
      } as never,
    })
