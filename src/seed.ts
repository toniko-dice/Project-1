 ...
 *
 * Скриптът е предвиден за ПРАЗНА база. Ако в нея вече има продукти, ОТКАЗВА
 * да се изпълни — иначе добавя дубликати и заместители върху истинско
 * съдържание. Заобикаля се само нарочно:  npm run seed -- --force
 *
 * Изображенията са автоматично генерирани заместители — заменете ги с
 * истинските продуктови снимки от админа.
 */
...
/*
  Защита срещу пускане върху пълна база.

  Продуктите са единственият сигурен белег: категории и страници се
  създават и от другите скриптове, а продукт в базата значи, че някой вече
  е въвеждал истинско съдържание. Скриптът не трие нищо, но създава нови
  записи със seed данни и заместители — върху работещ сайт това е
  подвеждащо и трудно за разчистване.

  Флагът е нарочно дълъг и изричен. Без него скриптът спира ПРЕДИ да е
  писал каквото и да е.
*/
const force = process.argv.includes('--force')
const { totalDocs: съществуващиПродукти } = await payload.count({ collection: 'products' })

if (съществуващиПродукти > 0 && !force) {
  console.error(
    `✗ В базата вече има ${съществуващиПродукти} продукта. Seed скриптът е за празна база и няма да продължи.`,
  )
  console.error('  Ако наистина искате да добавите примерното съдържание отгоре: npm run seed -- --force')
  process.exit(1)
}

if (force && съществуващиПродукти > 0) {
  console.warn(`⚠ --force: добавям примерно съдържание върху ${съществуващиПродукти} съществуващи продукта.`)
}
...
const upload = async (label: string, alt: string, w: number, h: number, bg?: string, fg?: string) => {
  ...
  const doc = await payload.create({
    ...
    file: {
      ...
      mimetype: 'image/png',
      /*
        Името минава през slugify, който транслитерира кирилицата.

        Преди тук стоеше `replace(/[^a-z0-9]+/g, '-')` — той изтриваше
        всяка кирилска буква и от „Големи поръчки" оставаше само тире.
        Четирите банера с еднакви размери получаваха едно и също име,
        оцеляваше един, а останалите три полета оставаха празни. Точно
        тези празни задължителни снимки после блокираха всеки запис на
        началната страница от админа.
      */
      name: `${slugify(label) || 'file'}-${w}x${h}.png`,
      size: data.length,
    },
  })
  return doc.id
    ...
    layout: [
      {
        ...
        autoplaySeconds: 6,
        // Банерът е слайдер; един слайд се показва без чертички и без смяна.
        slides: [
          {
            ...
            subheading:
              'Подгответе дома си за спиране на тока — и вземете подарък към поръчката.',
            note: '5 – 31 август',
            image: heroImg,
            cta: {
              label: 'Разгледайте офертите',
