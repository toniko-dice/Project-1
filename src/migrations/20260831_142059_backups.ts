...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`CREATE TABLE \`backups\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`label\` text NOT NULL,
  	\`trigger\` text DEFAULT 'ръчно',
  	\`includes_media\` integer DEFAULT true,
  	\`media_files\` numeric,
  	\`note\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`backups_updated_at_idx\` ON \`backups\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`backups_created_at_idx\` ON \`backups\` (\`created_at\`);`)
  await db.run(sql`CREATE UNIQUE INDEX \`backups_filename_idx\` ON \`backups\` (\`filename\`);`)
  ...
  `)
  await db.run(sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`backups_id\` integer REFERENCES backups(id);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  await db.run(sql`DROP TABLE \`backups\`;`)
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_payload_locked_documents_rels\`("id", "order", "parent_id", "path", "pages_id", "products_id", "categories_id", "menu_panels_id", "media_id", "testimonials_id", "awards_id", "users_id") SELECT "id", "order", "parent_id", "path", "pages_id", "products_id", "categories_id", "menu_panels_id", "media_id", "testimonials_id", "awards_id", "users_id" FROM \`payload_locked_documents_rels\`;`)
